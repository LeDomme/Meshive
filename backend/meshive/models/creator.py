...
class CreatorLink(Base):
    ...
    __table_args__ = (UniqueConstraint("creator_name", "kind", "label"),)

    id: Mapped[int] = mapped_column(primary_key=True)
    creator_profile_id: Mapped[int | None] = mapped_column(
        ForeignKey("creator_profiles.id", ondelete="SET NULL"), nullable=True, index=True
    )
    creator_name: Mapped[str] = mapped_column(String(255, collation="NOCASE"), index=True)
...
class CreatorMerge(Base):
    __tablename__ = "creator_merges"
    id: Mapped[int] = mapped_column(primary_key=True)
    target_profile_id: Mapped[int] = mapped_column(ForeignKey("creator_profiles.id"), index=True)
    source_profile_id: Mapped[int] = mapped_column(ForeignKey("creator_profiles.id"), index=True)
