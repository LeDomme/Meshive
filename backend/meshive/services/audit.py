...
def log_event(session: Session, actor: User, action: str, target_type: str, target_label: str, *, target_id: int | None = None, details: dict[str, object] | None = None) -> None:
    session.add(AuditEvent(actor_user_id=actor.id, actor_username=actor.username, action=action, target_type=target_type, target_id=target_id, target_label=target_label, details=details))
