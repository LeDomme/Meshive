...
const admin = { id: 1, username: "Admin", email: null, email_verified: false, role: "admin", is_active: true, must_change_password: false, role_definition: { id: 5, name: "Administrator", is_system: true, is_superuser: true }, permissions: ["users.manage", "roles.manage"], source_access: { all_sources: true, source_ids: [] } }
const limited = { ...admin, id: 2, username: "Member", role: "user", role_definition: { id: 2, name: "Member", is_system: true, is_superuser: false }, permissions: ["catalogue.view"], source_access: { all_sources: true, source_ids: [] } }
const roles = [
  { id: 5, name: "Administrator", description: "Full access", is_system: true, is_superuser: true, permission_keys: ["users.manage", "roles.manage"], user_count: 1 },
  { id: 2, name: "Member", description: "Library member", is_system: true, is_superuser: false, permission_keys: ["catalogue.view"], user_count: 1 },
]
...
async function mockApi(page: import("@playwright/test").Page, user = admin) {
  ...
  await page.route("**/api/admin/roles/permissions", route => route.fulfill({ json: ["catalogue.view", "roles.manage", "users.manage"] }))
...
test("administrator sees Users and Roles and system roles are read-only", async ({ page }) => {
  await mockApi(page)
...
test("custom roles and selected-source users submit the management API", async ({ page }) => {
  await mockApi(page)
  let roleCreated = false
  await page.route("**/api/admin/roles", async route => {
    if (route.request().method() === "POST") { roleCreated = true; await route.fulfill({ json: { ...roles[1], id: 9, name: "Custom" }, status: 201 }); return }
    await route.fulfill({ json: roleCreated ? [...roles, { ...roles[1], id: 9, name: "Custom", is_system: false, user_count: 0 }] : roles })
  })
  await page.route("**/api/admin/users", async route => {
    if (route.request().method() === "POST") { expect(route.request().postDataJSON()).toMatchObject({ all_sources: false, source_ids: [1] }); await route.fulfill({ json: { ...users[0], id: 3, username: "Selected", all_sources: false, source_ids: [1] }, status: 201 }); return }
...
test("users without management permissions cannot open management URLs", async ({ page }) => {
...
test("custom roles can be edited and deleted", async ({ page }) => {
  const custom = { ...roles[1], id: 9, name: "Custom", is_system: false, user_count: 0 }
  await mockApi(page)
  await page.route("**/api/admin/roles/9", async route => {
    expect(["PUT", "DELETE"]).toContain(route.request().method())
    await route.fulfill({ status: route.request().method() === "DELETE" ? 204 : 200, json: custom })
  })
  await page.route("**/api/admin/roles", route => route.fulfill({ json: [...roles, custom] }))
  await page.goto("/admin/roles")
