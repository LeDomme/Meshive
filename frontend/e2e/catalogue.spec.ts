...
test("saved views restore catalogue filters, source scope, and sorting", async ({ page }) => {
  ...
  const savedViewSelect = page.getByRole("button", { name: "Saved views" })
  await expect(savedViewSelect).toContainText("Saved views")
  await savedViewSelect.click()
  await expect(page.getByRole("option", { name: "Saved views" })).toHaveCount(0)
  await expect(page.getByRole("option", { name: "Default view" })).toBeVisible()
  await page.keyboard.press("Escape")
  await expect(page.getByRole("button", { name: "Rename" })).toHaveCount(0)
  await expect(page.getByRole("button", { name: "Delete" })).toHaveCount(0)
  await expect(page.locator(".catalogue-filters [aria-label='Saved views']")).toHaveCount(0)
  await expect(page.locator(".catalogue-meta [aria-label='Saved views']")).toBeVisible()
  page.once("dialog", dialog => dialog.accept("Ada collection"))
  await page.getByRole("button", { name: "Save view" }).click()
  await expect(savedViewSelect).toContainText("Ada collection")
  await expect(page.getByRole("button", { name: "Rename" })).toHaveCount(0)
  await expect(page.getByRole("button", { name: "Delete" })).toHaveCount(0)
  await savedViewSelect.click()
  ...
  await expect(savedViewSelect).toContainText("Saved views")
  await savedViewSelect.click()
  await page.getByRole("option", { name: "Ada collection" }).click()
  await expect(savedViewSelect).toContainText("Ada collection")
