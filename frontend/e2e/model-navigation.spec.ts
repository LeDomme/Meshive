...
const detail = (id: number) => ({ id, name: `Model ${id}`, variants: [], variant: null, creator: null, creator_links: [], franchise: null, series: null, collection: null, status: "available", source_id: 1, source_name: "Library", relative_path: `Model ${id}`, images: [], archives: [], archive_bundle_download_url: null, recent_scan_issues: [], archive_statistics: null, tags: [] })
...
test("next model preserves catalogue context for navigation and back link", async ({ page }) => {
  ...
  await page.route(/\/api\/models\/\d+\/navigation.*/, route => { navigationRequests.push(route.request().url()); return route.fulfill({ json: { previous: { id: 1, name: "Model 1", variants: [], variant: null }, next: { id: 2, name: "Model 2", variants: [], variant: null } } }) })
