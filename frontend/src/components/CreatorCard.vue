...
<template>
  <section v-if="creator" class="creator-card" aria-label="Creator">
    <RouterLink class="creator-artwork" :to="{ name: 'creator-detail', params: { id: creator.id } }"><img :src="creator.artwork?.url || '/favorite-fallbacks/favorite-creator.webp'" :alt="creator.artwork ? `${creator.display_name} artwork` : ''"></RouterLink>
    <div class="creator-card-content"><p class="eyebrow">Creator</p><RouterLink class="creator-name" :to="{ name: 'creator-detail', params: { id: creator.id } }">{{ creator.display_name }}</RouterLink><p v-if="creator.description" class="creator-description">{{ creator.description }}</p><div v-if="creator.links.length" class="creator-card-links"><a v-for="link in creator.links" :key="link.id" v-show="safeUrl(link.url)" :href="safeUrl(link.url)!" target="_blank" rel="noopener noreferrer">{{ link.label }} ↗</a></div><div class="creator-card-actions"><span>{{ creator.model_count }} {{ creator.model_count === 1 ? "model" : "models" }}</span><RouterLink :to="{ name: 'creator-detail', params: { id: creator.id } }">View all models</RouterLink></div></div>
  </section>
</template>
