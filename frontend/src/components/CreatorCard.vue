...
<template>
  <section v-if="creator" class="creator-card" aria-label="Creator">
    <div class="creator-card-top"><RouterLink class="creator-artwork" :to="catalogueTarget(creator.id)"><img :src="creator.artwork?.url || '/favorite-fallbacks/favorite-creator.webp'" :alt="creator.artwork ? `${creator.display_name} artwork` : ''"></RouterLink><div class="creator-card-content"><p class="eyebrow">Creator</p><RouterLink class="creator-name" :to="catalogueTarget(creator.id)">{{ creator.display_name }}</RouterLink><div class="creator-card-actions"><span>{{ creator.model_count }} {{ creator.model_count === 1 ? "model" : "models" }}</span><RouterLink :to="catalogueTarget(creator.id)">View all</RouterLink></div></div></div>
    <p v-if="creator.description" class="creator-description">{{ creator.description }}</p>
