...
<template>
  <main class="admin-shell">
    ...
    <section class="panel audit-filter-panel"><div class="panel-heading"><div><h2>Filter audit events</h2><p class="panel-copy">Narrow the timeline by event, user or time range.</p></div></div><form class="audit-filters" @submit.prevent="load(true)"><label><span>Action</span><select v-model="action" :disabled="loading"><option value="">All actions</option><option v-for="(label, key) in labels" :key="key" :value="key">{{ label }}</option></select></label><label><span>Actor</span><input v-model="actor" placeholder="Search username" :disabled="loading"></label><label><span>From</span><input v-model="fromAt" type="datetime-local" :disabled="loading"></label><label><span>To</span><input v-model="toAt" type="datetime-local" :disabled="loading"></label><div class="row-actions"><button class="primary-button" :disabled="loading">{{ loading ? "Loading…" : "Apply filters" }}</button><button class="secondary-button" type="button" :disabled="loading" @click="reset">Reset</button></div></form></section>
