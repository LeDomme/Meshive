...
async function getPreview() { if (!profile.value || !mergeSourceId.value) return; preview.value = await apiRequest<Preview>("/api/admin/creator-profiles/merge/preview", { method: "POST", body: JSON.stringify({ target_profile_id: profile.value.id, source_profile_ids: [mergeSourceId.value] }) }); if (!preview.value.artwork_conflict && !preview.value.link_conflicts.length) await applyMerge() }
