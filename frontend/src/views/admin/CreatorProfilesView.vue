...
async function load() { profile.value = null; preview.value = null; alias.value = ""; history.value = []; if (!props.profileId) return; try { const all = await apiRequest<Profile[]>("/api/admin/creator-profiles"); profiles.value = all; profile.value = all.find(item => item.id === props.profileId) ?? null; name.value = profile.value?.display_name ?? ""; description.value = profile.value?.description ?? ""; history.value = await apiRequest<MergeHistory[]>(`/api/admin/creator-profiles/${props.profileId}/merge-history`) } catch (cause) { error.value = cause instanceof ApiError ? cause.message : "Unable to load creator profile" } }
...
async function undoMerge(id: number) { await apiRequest(`/api/admin/creator-profiles/merges/${id}/undo`, { method: "POST" }); await load(); emit("changed") }
