...
const profile = ref<Profile | null>(null); const name = ref(""); const description = ref(""); const alias = ref(""); const mergeSourceId = ref<number | null>(null); const profiles = ref<Profile[]>([]); const preview = ref<Preview | null>(null); const history = ref<MergeHistory[]>([]); const error = ref(""); const saveState = ref<"idle" | "saving" | "saved">("idle"); const saveError = ref("")
...
async function save() { if (!profile.value || saveState.value === "saving") return; const payload = { id: profile.value.id, display_name: name.value, description: description.value || null }; saveState.value = "saving"; saveError.value = ""; try { if (props.onSave) await props.onSave(payload); else await apiRequest(`/api/admin/creator-profiles/${profile.value.id}`, { method: "PUT", body: JSON.stringify(payload) }); await load(); saveState.value = "saved"; if (saveStateTimer) clearTimeout(saveStateTimer); saveStateTimer = setTimeout(() => { saveState.value = "idle" }, 2500) } catch (cause) { saveState.value = "idle"; saveError.value = cause instanceof ApiError ? cause.message : "Unable to save creator profile" } }
